...
export class AzureContainerClient {
	...
	public async uploadBlob(options: Readonly<BlobOptions>): Promise<void> {
		try {
			await this._containerClient.uploadBlockBlob(
				options.blobName,
				options.blobStream,
				options.blobSize,
				{
			...
			this._log?.info(
				`[Azure:${this._containerClient.accountName}:${this._containerClient.containerName}:(UploadBlob)] - Blob "${options.blobName}" uploaded`
			);
		} catch (error) {
			throw new CoreError({
	...
	public async downloadBlob(blobName: string): Promise<BlobOptions> {
		try {
			...
			this._log?.info(
				`[Azure:${this._containerClient.accountName}:${this._containerClient.containerName}:(DownloadBlob)] - Blob "${blobName}" downloaded`
			);
			return {
				blobName,
	...
	public async deleteBlob(blobName: string): Promise<void> {
		try {
			...
			this._log?.info(
				`[Azure:${this._containerClient.accountName}:${this._containerClient.containerName}:(DeleteBlob)] - Blob "${blobName}" deleted`
			);
		} catch (error) {
			throw new CoreError({
	...
	public async getBlobInfo(blobName: string): Promise<Omit<BlobOptions, 'blobStream'>> {
		try {
			...
			this._log?.info(
				`[Azure:${this._containerClient.accountName}:${this._containerClient.containerName}:(GetBlobInfo)] - Blob "${blobName}" information retrieved`
			);
			return {
				blobName,
	...
	public async checkBlobExists(blobName: string): Promise<boolean> {
		try {
			...
			this._log?.info(
				`[Azure:${this._containerClient.accountName}:${this._containerClient.containerName}:(CheckBlobExists)] - Blob "${blobName}" exists: ${exists}`
			);
			return exists;
		} catch (error) {
			throw new CoreError({
	...
	public async listBlobs(): Promise<string[]> {
		try {
			...
			this._log?.info(
				`[Azure:${this._containerClient.accountName}] - List ${blobs.length} blobs in container ${this._containerClient.containerName}`
			);
	...
	public async listBlobsDetails(): Promise<Omit<BlobOptions, 'blobStream'>[]> {
		try {
			const blobs = await Array.fromAsync(
				this._containerClient.listBlobsFlat({
					includeMetadata: true
				})
			);
			this._log?.info(
				`[Azure:${this._containerClient.accountName}] - List ${blobs.length} blobs details in container ${this._containerClient.containerName}`
			);

...
export class AzureBlobServiceClientCreator {
	...
	public async listContainers(): Promise<string[]> {
		try {
			...
			this._log?.info(
				`[Azure:${this._blobServiceClient.accountName}:(ListContainers)] - ${containers.length} containers found`
			);
	...
	public async createContainer(containerName: string): Promise<void> {
		try {
			...
			this._log?.info(
				`[Azure:${this._blobServiceClient.accountName}:(CreateContainer)] - Container "${containerName}" created`
			);
		} catch (error) {
			throw new CoreError({
	...
	public async deleteContainer(containerName: string): Promise<void> {
		try {
			...
			this._log?.info(
				`[Azure:${this._blobServiceClient.accountName}:(DeleteContainer)] - Container "${containerName}" deleted`
			);
		} catch (error) {
			throw new CoreError({

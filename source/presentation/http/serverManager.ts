...
import {
	...
	type FastifyReply,
	type FastifyRequest
} from '#/common/lib/required/fastify/fastify.lib.ts';
...
export class ServerManager {
	...
	public constructor(options?: Partial<ServerOptions>) {
		...
		this._app = fastify({
			...
			ajv: {
				...
				plugins: [ajvError.default, ajvFormats.default, ajvFilePlugin]
			}
		});
	...
	public async addPlugins(plugins: Plugin[]): Promise<void> {
		await Promise.all(
			plugins.map((plugin) => plugin.configure(this._app, this._options.baseUrl))
		);
	}
	...
	public async addRouters(routers: AbstractRouter[]): Promise<void> {
		await Promise.all(
			routers.map((router) => router.configure(this._app, this._options.baseUrl))
		);
	}
	...
	public async start(startOptions?: Partial<StartOptions>): Promise<void> {
		...
		if (this._options.logger)
			this._options.logger.info(
				`Server listening on ${this._startOptions.host}:${this._startOptions.port}`
			);
	}
	 ...
	 */
	private async _handleValidationErrors(
		error: FastifyError,
			...
			if (instancePath === '' && params && missingProperty)
				return {
					...
					message: I18n.isI18nInitialized()
						? I18n.translate(keyword, request.headers['accept-language'], {
								property: missingProperty
							})
						: keyword,
					contraints: params
	...
	private _deepSearchKey(
		...
		if (
			keyToFind in obj &&
			(typeof obj[keyToFind] === 'string' || typeof obj[keyToFind] === 'number')
		)
		...
		for (const value of Object.values(obj)) {
			if (Array.isArray(value)) {
				for (const item of value) {
					...
					const found: number | string | undefined = this._deepSearchKey(
	...
	private _handleFileErrors(
		...
		const isTooLarge: boolean =
			(multipartCode === 'FST_REQ_FILE_TOO_LARGE' ||
				error.message === 'request file too large') &&
			maxFileSize !== undefined;
		const isLimitReached: boolean =
			(multipartCode === 'FST_FILES_LIMIT' || error.message === 'reach files limit') &&
			maxItems !== undefined;

		if (!isTooLarge && !isLimitReached) return null;

		const status: number = isTooLarge ? 413 : isLimitReached ? 400 : 500;
	 ...
	 * @param reply - The reply. ({@link FastifyReply})
	 */
	private async _setErrorHandler(
		error: FastifyError,
		request: FastifyRequest,
		reply: FastifyReply
	): Promise<void> {
		if (this._options.logger) this._options.logger.error(error);
		...
		const message = I18n.isI18nInitialized()
			? I18n.translate(ErrorKeys.INTERNAL_SERVER_ERROR, request.headers['accept-language'])
			: ErrorKeys.INTERNAL_SERVER_ERROR;
	...
	private _addDefaultHooks(): void {
		if (this._options.logger) new LoggerHook(this._options.logger).configure(this._app);

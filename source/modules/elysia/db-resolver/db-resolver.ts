		...
		.macro({
			injectDynamicDB(config: Omit<MSSQLDatabaseOptions, 'databaseName'>) {
				return {
					...
					async resolve({ headers }): Promise<{ dynamicDB: MSSQL }> {
						...
						if (!SingletonManager.has(`${prefixDatabaseName}${databaseName}`)) {
							SingletonManager.register(
								`${prefixDatabaseName}${databaseName}`,
								new MSSQL({
									...config,
									databaseName
								})
							);
							await SingletonManager.get<MSSQL>(
								`${prefixDatabaseName}${databaseName}`
							).connect();
						}
						return {
							dynamicDB: SingletonManager.get<MSSQL>(
								`${prefixDatabaseName}${databaseName}`
							)
						};
					}
			...
			injectStaticDB(databaseName: string) {
				return {
					resolve(): { staticDB: MSSQL } {
						if (!SingletonManager.has(`${prefixDatabaseName}${databaseName}`))
							throw new InternalError(
								DB_RESOLVER_ERROR_KEYS.DB_RESOLVER_STATIC_DB_NOT_FOUND,
								`${prefixDatabaseName}${databaseName}`
							);

						return {
							staticDB: SingletonManager.get<MSSQL>(
								`${prefixDatabaseName}${databaseName}`
							)
						};
					}

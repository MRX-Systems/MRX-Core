...
import { RATE_LIMIT_ERROR_KEYS } from '#/modules/elysia/rate-limit/enums';
import { rateLimitPlugin } from '#/modules/elysia/rate-limit/rate-limit';
...
describe.concurrent('rateLimit', () => {
	...
	test('should return 429 when rate limit is exceeded', async () => {
		...
		expect(parseInt(blockedResponse.headers.get('X-RateLimit-Reset') || '0')).toBeGreaterThan(
			0
		);
	});
	...
	test('should handle different rate limits for different routes', async () => {
		...
		for (let i = 0; i < 2; ++i)
			await app.handle(
				new Request('http://localhost/route1', {
					headers: { 'x-forwarded-for': ip }
				})
			);

		// Next request to route1 should be blocked
		const blockedResponse1 = await app.handle(
			new Request('http://localhost/route1', {
				headers: { 'x-forwarded-for': ip }
			})
		);
		...
		expect(parseInt(blockedResponse1.headers.get('X-RateLimit-Reset') || '0')).toBeGreaterThan(
			0
		);
		...
		for (let i = 0; i < 3; ++i) {
			const response = await app.handle(
				new Request('http://localhost/route2', {
					headers: { 'x-forwarded-for': ip }
				})
			);
			expect(response.status).toBe(200);

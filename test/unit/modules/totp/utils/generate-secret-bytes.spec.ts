...
describe('generateSecretBytes', () => {
	test.concurrent('should generate 20 random bytes, is default option', () => {
		const result = generateSecretBytes();
		expect(result).toBeInstanceOf(Uint8Array);
		expect(result.length).toBe(20);
	});

	test.concurrent('should generate specified number of random bytes', () => {
		const length = 32;
	...
	test.concurrent('should throw error for zero length', () => {
		expect(() => generateSecretBytes(0)).toThrow(TOTP_ERROR_KEYS.INVALID_SECRET_LENGTH);
	});

	test.concurrent('should throw error for negative length', () => {
